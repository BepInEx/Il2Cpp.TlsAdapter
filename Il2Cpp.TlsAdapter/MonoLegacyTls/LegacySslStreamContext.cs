...
namespace Il2Cpp.TlsAdapter.MonoLegacyTls
{
    public class LegacySslStreamContext : MobileTlsContext
    {
        ...
        private SslStreamBase sslStream;
        ...
        private X509Certificate OnCertificateSelection(X509CertificateCollection clientCerts,
            X509Certificate serverCert, string targetHost, X509CertificateCollection serverRequestedCerts)
        {
#pragma warning disable 618
            var acceptableIssuers = new string [serverRequestedCerts?.Count ?? 0];
            for (var i = 0; i < acceptableIssuers.Length; i++)
                if (serverRequestedCerts != null)
                    acceptableIssuers[i] = serverRequestedCerts[i].GetIssuerName();
            certificateValidator.SelectClientCertificate(targetHost, clientCerts, serverCert, acceptableIssuers,
                out var clientCertificate);
            return clientCertificate;
#pragma warning restore 618
        }

        private TlsSecurityProtocolType GetMonoSslProtocol(SslProtocols ms)
        {
            switch (ms)
            {
        ...
        public override bool ProcessHandshake()
        {
            ...
            return handshakeResult == null || handshakeResult.IsCompleted ||
                   sslStream.context.HandshakeState == HandshakeState.Finished;
        }
