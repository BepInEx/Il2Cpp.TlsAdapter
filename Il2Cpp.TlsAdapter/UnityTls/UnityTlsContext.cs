...
namespace Mono.Unity
{
	unsafe internal class UnityTlsContext : MobileTlsContext
	{
			...
			: base (parent, opts)
		{
			...
			if (opts.ServerMode) {
				ExtractNativeKeyAndChainFromManagedCertificate(opts.ServerCertificate, &errorState, out var serverCerts, out var serverPrivateKey);
				try {
				...
				byte [] targetHostUtf8 = Encoding.UTF8.GetBytes (opts.TargetHost);
		...
		public override bool IsRemoteCertificateAvailable {
