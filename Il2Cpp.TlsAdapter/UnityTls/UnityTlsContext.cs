...
{
    internal unsafe class UnityTlsContext : MobileTlsContext
    {
...
#pragma warning restore CS0162 // Reenable unreachable code warning.
        ...
        public override (int ret, bool wantMore) Read(byte[] buffer, int offset, int count)
        {
            ...
            {
                numBytesRead = (int) UnityTls.NativeInterface.unitytls_tlsctx_read(tlsContext, bufferPtr + offset,
                    (size_t) count, &errorState);
            }
            ...
            switch (errorState.code)
            {
                ...
                case UnityTls.unitytls_error_code.UNITYTLS_STREAM_CLOSED:
                    return
                        (0, false); // According to Apple and Btls implementation this is how we should handle gracefully closed connections.

                default:
                    if (!closedGraceful) Unity.Debug.CheckAndThrow(errorState, "Failed to read data to TLS context");
                    return (0, false);
            }
        }
        ...
        public override (int ret, bool wantMore) Write(byte[] buffer, int offset, int count)
        {
            ...
            fixed (byte* bufferPtr = buffer)
            {
                numBytesWritten = (int) UnityTls.NativeInterface.unitytls_tlsctx_write(tlsContext, bufferPtr + offset,
                    (size_t) count, &errorState);
            }
            ...
            switch (errorState.code)
            {
                ...
                default:
                    Unity.Debug.CheckAndThrow(errorState, "Failed to write data to TLS context");
                    return (0, false);
            }
        }
        ...
        public override void StartHandshake()
        {
            if (Settings != null && Settings.EnabledCiphers != null)
            {
                ...
                {
                    UnityTls.NativeInterface.unitytls_tlsctx_set_supported_ciphersuites(tlsContext, ciphersPtr,
                        (size_t) ciphers.Length, &errorState);
                }

                Unity.Debug.CheckAndThrow(errorState, "Failed to set list of supported ciphers",
        ...
        private static size_t WriteCallback(void* userData, byte* data, size_t bufferLen,
